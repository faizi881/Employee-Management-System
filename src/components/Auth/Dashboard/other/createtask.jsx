        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
          <div className="col-span-1 flex flex-col gap-4">
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Make a UI design" className="w-full px-3 py-2 rounded-lg bg-[#0b0f14] border border-gray-700 text-gray-100 
                       focus:outline-none focus:border-emerald-500/70 text-sm" />
            </div>
            <div>
              <h3 className="text-gray-300 mb-1 text-sm">Date</h3>
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-[#0b0f14] border border-gray-700 text-gray-100 
                       focus:outline-none focus:border-emerald-500/70 text-sm" />
            </div>
            <div>
              <h3 className="text-gray-300 mb-1 text-sm">Assign To</h3>
              <select value={assigneeId} onChange={(e) => setAssigneeId(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-[#0b0f14] border border-gray-700 text-gray-100 
                       focus:outline-none focus:border-emerald-500/70 text-sm">
            ...
            <div>
              ...
              <input type="text" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Design, Development, Testing" className="w-full px-3 py-2 rounded-lg bg-[#0b0f14] border border-gray-700 text-gray-100 
                       focus:outline-none focus:border-emerald-500/70 text-sm" />
            </div>
            <button disabled={!canSubmit} type="submit" className="mt-1 bg-gradient-to-r from-emerald-600 to-emerald-500 disabled:opacity-50 hover:from-emerald-500 hover:to-emerald-400 transition-colors text-white py-2 
          ...
          <div className="col-span-1">
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the task" className="w-full h-[280px] px-3 py-2 rounded-lg bg-[#0b0f14] border border-gray-700   
                     text-gray-100 focus:outline-none focus:border-emerald-500/70 resize-none text-sm"></textarea>
          </div>
        </form>
      </div>
    </>
